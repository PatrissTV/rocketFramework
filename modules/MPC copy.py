...
class MPC:
    ...
    def model(self):
        ...
        #self.m.tf = Var(domain=NonNegativeReals, initialize=10)
    ...
    def constraints(self):
        tf = self.tf
        self.m.ode1 = Constraint(self.m.t, rule=lambda m, t: m.ddx1[t] == m.u1[t])
        self.m.ode2 = Constraint(self.m.t, rule=lambda m, t: m.ddx2[t] == m.u2[t])
        self.m.ode3 = Constraint(self.m.t, rule=lambda m, t: m.ddx3[t] == m.u3[t] - 9.81)

        #self.m.thrus1 = Constraint(self.m.t, rule=lambda m, t: m.u1[t]**2 + m.u2[t]**2 + m.u3[t]**2 <= 20**2)
        self.m.thrus2 = Constraint(self.m.t, rule=lambda m, t: m.u1[t] >= 0)
        self.m.thrus3 = Constraint(self.m.t, rule=lambda m, t: m.u2[t] >= 0)
        self.m.thrus4 = Constraint(self.m.t, rule=lambda m, t: m.u3[t] >= 0)
        ...
        self.m.final4 = Constraint(rule=lambda m: m.dx1[tf]**2 <= 1)
        self.m.final5 = Constraint(rule=lambda m: m.dx2[tf]**2 <= 1)
        self.m.final6 = Constraint(rule=lambda m: m.dx3[tf]**2 <= 1)
